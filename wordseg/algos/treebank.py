"""Reads, searches and displays trees from Penn Treebank (PTB) format
...
def _string_trees(trees, s, pos=0):
    """Reads a sequence of trees in string s[pos:]. Appends the trees to
    the argument trees. Returns the ending position of those trees in s.
...
#     """trees is a generator that iterates through the trees in a treebank.
#     treebankdir is the name of the directory in which the treebank
#     files are located, and treebank_re is a regular expression that
#     all treebank files must match.
...
#     treebankdir and treebank_re specify the treebank files to be
#     searched.  interesting_tree_p is a function which is called on
#     each tree in turn.  If interesting_tree_p does not return False,
#     the tree is displayed using drawtree.
...
#                 inp = raw_input(
#                     filename + ":" + str(treeno) +
#                     " find next tree? [Y/n/p] ").strip()

#                 if len(inp) != 0:
#                     if inp[0] == "P" or inp[0] == "p":
#                         psfilename = raw_input(
#                             "Saving postscript image of tree; "
#                             "enter filename for image file? ").strip()
#                         if psfilename != "":
...
#     treebankdir and treebank_re specify which treebank files are to be
#     searched.
...
def is_preterminal(subtree):
    ...
    return (isinstance(subtree, list)
            ...
            and functools.reduce(
                lambda x, y: x and is_terminal(y), subtree[1:], True))


def is_phrasal(subtree):
    """True if this treebank subtree is not a terminal or a preterminal node"""
    return (isinstance(subtree, list)
            and (len(subtree) == 1 or isinstance(subtree[1], list)))
