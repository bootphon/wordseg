...
namespace pstream
{
    ...
    // {
    //     FILE* stdio_fp;
    //     // io::stream_buffer<io::file_descriptor_source> stdio_fb;
    //     // std::filebuf stdio_fb;
    //     __gnu_cxx::stdio_filebuf<char> stdio_fb;

    //     istream_helper(const char* command)
    //         : stdio_fp(popen(command, "r")),
    //           stdio_fb(fileno(stdio_fp), std::ios_base::in)
    //         {}
    ...
    // // izstream_helper{} exists to compute the command that istream
    // // has to execute while izstream is being constructed.
    ...
    // // An izstream popen's a set of files, running them through zcat,
    // // bzcat or cat depending on the suffix of the last file.
    ...
    struct ostream_helper
    {
        ...
        // __gnu_cxx::stdio_filebuf<char> stdio_fb;
        // std::filebuf stdio_fb;
        io::stream<io::file_descriptor_sink> stdio_fb;

        ostream_helper(const char* command)
            : stdio_fp(popen(command, "w")),
              stdio_fb(fileno(stdio_fp)) //, std::ios_base::out)
            {}
    ...
    // Instantiating ostream_flushcout{} flushes std::cout, which
    // should be done before a popen or fork command (the output
    // buffer is shared between processes)
    ...
    using boost_stream = io::stream_buffer<io::file_descriptor_sink>;

    class ostream : ostream_helper, ostream_flushcout, boost_stream
    {
        FILE* stdio_fp;

        ostream(const std::string& command)
            : ostream_helper(command.c_str()),
              boost_stream(stdio_fp)
            {}
