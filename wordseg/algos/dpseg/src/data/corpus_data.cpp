...
Sentences data::corpus_data::get_eval_sentences(double init_pboundary, double aeos) const
{
    Sentences s;
    std::vector<bool> possible_bs;
    std::vector<bool> true_bs;
    for (uint i = _evalsent_start; i < m_sentenceboundaries.size()-1; ++i)
    {
...
void data::corpus_data::read(std::wistream& is, std::size_t start, std::size_t ns)
{
...
void data::corpus_data::read_eval(std::wistream& is, std::size_t start, std::size_t ns)
{
    _evalsent_start = m_sentenceboundaries.size()-1;
    read_data(is, start, ns);
}

void data::corpus_data::read_data(std::wistream& is, std::size_t start, std::size_t ns)
{
...
void data::corpus_data::initialize(std::size_t ns)
{
    m_ntrainsentences = ns;
    if (m_ntrainsentences == 0)
    {
        if (_evalsent_start > 0)
            m_ntrainsentences = _evalsent_start;
        else
            m_ntrainsentences = m_sentenceboundaries.size()-1;
    }
